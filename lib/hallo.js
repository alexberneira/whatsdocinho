...
class HalloAPI {
  constructor() {
    ...
    this.baseURL = isDevelopment 
      ? (process.env.NEXT_PUBLIC_HALLO_API_URL || 'http://localhost:3001/api/hallo')
  ...
  async sendTextMessage(phoneNumber, message) {
    ...
    try {
      // Construir URL para a nova API Route com query parameters
      const apiUrl = `${this.baseURL}?instance=${this.instanceName}&token=${this.apiKey}&action=message`;
