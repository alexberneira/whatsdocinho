...
class HalloAPI {
  ...
  // Método genérico para enviar mensagem (texto, imagem, vídeo ou documento)
  async sendMessage(messageData) {
    const { number, text, image, video, document } = messageData;
    ...
    console.log('=== DEBUG HALLO API (SEND MESSAGE) ===');
    console.log('URL da requisição:', apiUrl);
    console.log('Número formatado:', formattedNumber);
    ...
    try {
      ...
      // Determinar o tipo de ação baseado no conteúdo
      if (image) {
        formData.append("ACTION", "IMAGE");
        // Para imagens, converter URL para base64
        try {
          console.log('🔄 Convertendo imagem para base64...');
          const imageBase64 = await this.urlToBase64(image);
          const imageName = this.extractFileName(image);
          formData.append("image_base64", imageBase64);
          if (imageName) {
            formData.append("image_name", imageName);
          }
      ...
      console.log('📦 FormData criado:', {
        ...
        // outros campos dependem do tipo
      });
